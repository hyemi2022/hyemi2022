...
# Selected Feature
corr_pears_hex_para = x_data.corrwith(Train['HEX_KA'], method='pearson')
corr_pears_hex_para = abs(corr_pears_hex_para)
indices = np.argsort(corr_pears_hex_para)[::-1]
...
for f in range(x_data.shape[1]):
   print("%2d) %-*s %f" % (f + 1, 30, 
                            feature_names[indices[f]], corr_pears_hex_para[indices[f]]))

   Feature.append(feature_names[indices[f]])
   Correlation_feature.append(corr_pears_hex_para[indices[f]])
...
for i in range(kfold):
    ...
    mc = ModelCheckpoint('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_HEX_KA.h5', 
    ...
    best_model_y01.save('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_HEX_KA.h5')
...
mc=ModelCheckpoint('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_HEX_KA.h5', 
                        monitor='val_loss', mode='min', verbose=1, save_best_only = True)
MLP_Model = load_model('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_HEX_KA.h5')
...
with open('Result_Estimation.csv', 'a', newline="") as f:
    writer = csv.writer(f)
    writer.writerow(['Pearson_Case02', loss, acc, Result_KA_R2, Result_KA_RMSE, Result_KA_MAE, Result_KA_MBE])
...
with open('ResultTempHEX_Estimation.csv', 'a', newline="") as f:
    writer = csv.writer(f)
    writer.writerow(['SOWT_Pearson_Case02', Result_SOWT_R2, Result_SOWT_RMSE, Result_SOWT_MAE, Result_SOWT_MBE])
    writer.writerow(['LOWT_Pearson_Case02', Result_LOWT_R2, Result_LOWT_RMSE, Result_LOWT_MAE, Result_LOWT_MBE])
