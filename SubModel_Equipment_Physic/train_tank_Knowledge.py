...
# y_train data --> Tank Parameters 'KA'
...
# train data setting
feature_names = ['ST01_SITemp', 'ST01_LITemp', 'ST01_Sflow', 'ST01_Lflow']
...
for i in range(kfold):
    ...
    mc = ModelCheckpoint('C:\\System_Air\\best_model\\physics_parameter\\Knowledge_Tank_parameter.h5', 
    ...
    best_model_y01.save('C:\\System_Air\\best_model\\physics_parameter\\Knowledge_Tank_parameter.h5')
...
x_predict_01 = Predict[feature_names]
...
mc=ModelCheckpoint('C:\\System_Air\\best_model\\physics_parameter\\Knowledge_Tank_parameter.h5', 
                        monitor='val_loss', mode='min', verbose=1, save_best_only = True)
y01_Model = load_model('C:\\System_Air\\best_model\\physics_parameter\\Knowledge_Tank_parameter.h5')
history = y01_Model.fit(x_train, UAs_Para, epochs=1000, batch_size=12240, validation_split=0.2, verbose=1, callbacks=[es, mc])
...
for i in range(len(SITemp_train)):
    ...
    UAs = Predict['Tank_Para_Predict'].iloc[i]/10          # kJ/hk [kJ/hr.m2.K]???
    ...
    # print(i, SITemp, '10소(저온)', Node10_2nd, Node9_2nd, Node2_2nd, Tank_SF, LITemp, '1소(고온)', Node1_2nd, Tank_LF)
...
submission.to_csv('Tank_prediction.csv', index = False)
...
with open('Result_Estimation.csv', 'a', newline="") as f:
    writer = csv.writer(f)
    writer.writerow(['Tank_US', loss, acc, Result_Para_R2, Result_Para_RMSE, Result_Para_MAE, Result_Para_MBE])
...
with open('ResultTempTank_Estimation.csv', 'a', newline="") as f:
    writer = csv.writer(f)
    writer.writerow(['Layer01', Result_Layer01_R2, Result_Layer01_RMSE, Result_Layer01_MAE, Result_Layer01_MBE])
    writer.writerow(['Layer10', Result_Layer10_R2, Result_Layer10_RMSE, Result_Layer10_MAE, Result_Layer10_MBE])
