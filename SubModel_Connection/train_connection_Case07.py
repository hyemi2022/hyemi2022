...
feature_names = ['R01_Flow', 'R02_Flow', 'HEX01_SFlow', 'HEX01_LFlow', 'ST01_Sflow', 'ST01_Lflow', 'SH01_Flow',
                'Week', 'Timepermin', 'Outdoor_Temperature', 'Outdoor_Humidity', 'Secondary_Load',
                'R01_OTemp', 'R02_OTemp', 'HEX01_SOTemp', 'HEX01_LOTemp', 'ST01_SOTemp', 'ST01_LOTemp', 'RH01_Temp']
...
def Classifiation_MLP():
    ...
    model.add(Dense(100, input_dim = 19, activation = 'sigmoid'))
...
mc = ModelCheckpoint('C:\\System_Air\\best_model\\connection\\Case07.h5', 
...
Result_Estimation = np.array([['Case07', loss, acc, Result_RMSE, Result_R2]])
...
Result_Predict.to_csv('Result_Predict_Case07.csv', index=False)
Encoded_Train.to_csv('Encoded_Train_Case07.csv', index=False)
Encoded_Predict.to_csv('Encoded_Predict_Case07.csv', index=False)
Prediction.to_csv('Prediction_Label_Case07.csv', index=False)

with open('Result_Estimation.csv', 'a', newline="") as f:
    writer = csv.writer(f)
    writer.writerow(['Case07', loss, acc, Result_R2, Result_RMSE, Result_MAE, Result_MBE])
