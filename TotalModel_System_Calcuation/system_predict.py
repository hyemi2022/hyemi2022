...
# All Black
Rc01_PumpFlow_model = load_model('C:\\System_Air\\best_model\\IVS_Case\\Spearman20_flow_y01.h5')
Rc02_PumpFlow_model = load_model('C:\\System_Air\\best_model\\IVS_Case\\Spearman20_flow_y02.h5')
Tank_PumpFlow_model = load_model('C:\\System_Air\\best_model\\IVS_Case\\Spearman20_flow_y03.h5')
HEX_PumpFlow_model = load_model('C:\\System_Air\\best_model\\IVS_Case\\Spearman20_flow_y04.h5')
Load_PumpFlow_model = load_model('C:\\System_Air\\best_model\\IVS_Case\\Spearman20_flow_y05.h5')
...
Rc01_LITemp = Predict['R01_ITemp_1bef'].loc[0] 
Rc02_LITemp = Predict['R02_ITemp_1bef'].loc[0]
HEX_LITemp = Predict['HEX01_LITemp_1bef'].loc[0]
HEX_SITemp = Predict['HEX01_SITemp_1bef'].loc[0]
Tank_SITemp = Predict['ST01_SITemp_1bef'].loc[0]
Tank_LITemp = Predict['ST01_LITemp_1bef'].loc[0]
RHeader_Temp = Predict['RH01_Temp_1bef'].loc[0]
SHeader_Temp = Predict['SH01_Temp_1bef'].loc[0]
...
# Input Variables for flow prediction ANN Model
R01Pump_Control = Predict['R01Pump_Control'].loc[0]
R02Pump_Control = Predict['R02Pump_Control'].loc[0]
TankPump_Control = Predict['ST01Pump_Control'].loc[0]
HEXPump_Control = Predict['HEX01Pump_Control'].loc[0]
LoadPump_Control = Predict['2ndPump_Control'].loc[0]

R01Pump_ONOFF = Predict['R01Pump_ONOFF'].loc[0]
R02Pump_ONOFF = Predict['R02Pump_ONOFF'].loc[0]
TankPump_ONOFF = Predict['ST01Pump_ONOFF'].loc[0]
HEXPump_ONOFF = Predict['HEX01Pump_ONOFF'].loc[0]
LoadPump_ONOFF = Predict['2ndPump_ONOFF'].loc[0]
...
Rc01_PumpFlow_input = (Load_2nd, Tank_Storage_1st, R01_OTemp_1st, Rc01_LITemp)
Rc01_PumpFlow_input = np.asarray(Rc01_PumpFlow_input).reshape(1, 4)
Rc02_PumpFlow_input = (R02_OTemp_1st, Tank_SITemp, Tank_Storage_1st, HEX01_LOTemp_1st)
Rc02_PumpFlow_input = np.asarray(Rc02_PumpFlow_input).reshape(1, 4)
Tank_PumpFlow_input = (Load_2nd, Tank_Storage_1st, HEX01_SOTemp_1st, HEX01_LOTemp_1st)
Tank_PumpFlow_input = np.asarray(Tank_PumpFlow_input).reshape(1, 4)
HEX_PumpFlow_input = (Load_2nd, Tank_Storage_1st, HEX01_SOTemp_1st, HEX01_LOTemp_1st)
HEX_PumpFlow_input = np.asarray(HEX_PumpFlow_input).reshape(1, 4)
Load_PumpFlow_input = (Load_2nd, Tank_Storage_1st, HEX01_SOTemp_1st, HEX01_LOTemp_1st)
Load_PumpFlow_input = np.asarray(Load_PumpFlow_input).reshape(1, 4)
    # prediction from ANN
...
for i in range(Outdoor_Temperature) :
    ...
    R01Pump_Control_2nd = Predict['R01Pump_Control'].iloc[i]
    R02Pump_Control_2nd = Predict['R02Pump_Control'].iloc[i]
    TankPump_Control_2nd = Predict['ST01Pump_Control'].iloc[i]
    HEXPump_Control_2nd = Predict['HEX01Pump_Control'].iloc[i]
    LoadPump_Control_2nd = Predict['2ndPump_Control'].iloc[i]

    R01Pump_ONOFF_2nd = Predict['R01Pump_ONOFF'].iloc[i]
    R02Pump_ONOFF_2nd = Predict['R02Pump_ONOFF'].iloc[i]
    TankPump_ONOFF_2nd = Predict['ST01Pump_ONOFF'].iloc[i]
    HEXPump_ONOFF_2nd = Predict['HEX01Pump_ONOFF'].iloc[i]
    LoadPump_ONOFF_2nd = Predict['2ndPump_ONOFF'].iloc[i]


    # flow
    # combinate input data
    
    Rc01_PumpFlow_input_2nd = (Load_2nd_2nd, Tank_Storage_2nd, Rc01_LOTemp_2nd, Rc01_LITemp_2nd)
    Rc01_PumpFlow_input_2nd = np.asarray(Rc01_PumpFlow_input_2nd).astype(np.float32)
    Rc01_PumpFlow_input_2nd = Rc01_PumpFlow_input_2nd.reshape(1, 4)
    Rc02_PumpFlow_input_2nd = (Rc02_LOTemp_2nd, Tank_SITemp_2nd, Tank_Storage_2nd, HEX_LOTemp_2nd)
    Rc02_PumpFlow_input_2nd = np.asarray(Rc02_PumpFlow_input_2nd).astype(np.float32)
    Rc02_PumpFlow_input_2nd = Rc02_PumpFlow_input_2nd.reshape(1, 4)
    Tank_PumpFlow_input_2nd = (Load_2nd_2nd, Tank_Storage_2nd, HEX_SOTemp_2nd, HEX_LOTemp_2nd)
    Tank_PumpFlow_input_2nd = np.asarray(Tank_PumpFlow_input_2nd).astype(np.float32)
    Tank_PumpFlow_input_2nd = Tank_PumpFlow_input_2nd.reshape(1, 4)
    HEX_PumpFlow_input_2nd = (Load_2nd_2nd, Tank_Storage_2nd, HEX_SOTemp_2nd, HEX_LOTemp_2nd)
    HEX_PumpFlow_input_2nd = np.asarray(HEX_PumpFlow_input_2nd).astype(np.float32)
    HEX_PumpFlow_input_2nd = HEX_PumpFlow_input_2nd.reshape(1, 4)
    Load_PumpFlow_input_2nd = (Load_2nd_2nd, Tank_Storage_2nd, HEX_SOTemp_2nd, HEX_LOTemp_2nd)
    Load_PumpFlow_input_2nd = np.asarray(Load_PumpFlow_input_2nd).astype(np.float32)
    Load_PumpFlow_input_2nd = Load_PumpFlow_input_2nd.reshape(1, 4)
    # Prediction by ANN
