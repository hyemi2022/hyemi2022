...
# Selected Feature
corr_pears_p01_para = x_data.corrwith(Train['p01_Parameter'], method='pearson')  # Pearson Correlation
corr_pears_p01_para = abs(corr_pears_p01_para)
indices_01 = np.argsort(corr_pears_p01_para)[::-1]

Feature_01 = []
Correlation_feature_01 = []
for f in range(x_data.shape[1]):
    print("%2d) %-*s %f" % (f + 1, 5, 
                            feature_names[indices_01[f]], corr_pears_p01_para[indices_01[f]]))

    Feature_01.append(feature_names[indices_01[f]])
    Correlation_feature_01.append(corr_pears_p01_para[indices_01[f]])
...
for i in range(kfold):
    ...
    mc = ModelCheckpoint('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_p01_parameter.h5', 
    ...
    best_model_y01.save('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_p01_parameter.h5')
...
mc=ModelCheckpoint('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_p01_parameter.h5', 
                        monitor='val_loss', mode='min', verbose=1, save_best_only = True)
y01_Model = load_model('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_p01_parameter.h5')
...
# Selected Feature
corr_pears_p02_para = x_data.corrwith(Train['p02_Parameter'], method='pearson')
corr_pears_p02_para = abs(corr_pears_p02_para)
indices_02 = np.argsort(corr_pears_p02_para)[::-1]

Feature_02 = []
Correlation_feature_02 = []
for f in range(x_data.shape[1]):
    print("%2d) %-*s %f" % (f + 1, 30, 
                            feature_names[indices_02[f]], corr_pears_p02_para[indices_02[f]]))

    Feature_02.append(feature_names[indices_02[f]])
    Correlation_feature_02.append(corr_pears_p02_para[indices_02[f]])
...
for i in range(kfold):
    ...
    es = EarlyStopping(monitor = 'val_loss', patience=30, mode= 'min', min_delta=0.001, verbose=1)
    mc = ModelCheckpoint('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_p02_parameter.h5', 
                            monitor='val_loss', mode='min', verbose=1, save_best_only=True)

    best_model_y02 = keras_model_02()
    best_model_y02.fit(partial_train_data_02, partial_train_targets_02, epochs=30, batch_size=9000)
    best_model_y02.save('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_p02_parameter.h5')
...
#### best Fitting Model ####
es=EarlyStopping(monitor='val_loss', patience=30, mode= 'min', min_delta=0.001, verbose=1)
mc=ModelCheckpoint('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_02_parameter.h5', 
                        monitor='val_loss', mode='min', verbose=1, save_best_only = True)
y02_Model = load_model('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_p02_parameter.h5')
...
# Selected Feature
corr_pears_p03_para = x_data.corrwith(Train['p03_Parameter'], method='pearson')
corr_pears_p03_para = abs(corr_pears_p03_para)
indices_03 = np.argsort(corr_pears_p03_para)[::-1]

Feature_03 = []
Correlation_feature_03 = []
for f in range(x_data.shape[1]):
    print("%2d) %-*s %f" % (f + 1, 30, 
                            feature_names[indices_03[f]], corr_pears_p03_para[indices_03[f]]))

    Feature_03.append(feature_names[indices_03[f]])
    Correlation_feature_03.append(corr_pears_p03_para[indices_03[f]])
...
for i in range(kfold):
    ...
    es = EarlyStopping(monitor = 'val_loss', patience=30, mode= 'min', min_delta=0.001, verbose=1)
    mc = ModelCheckpoint('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_p03_parameter.h5', 
                            monitor='val_loss', mode='min', verbose=1, save_best_only=True)

    best_model_y03 = keras_model_03()
    best_model_y03.fit(partial_train_data_03, partial_train_targets_03, epochs=30, batch_size=9000)
    best_model_y03.save('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_p03_parameter.h5')
...
mc=ModelCheckpoint('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_p03_parameter.h5', 
                        monitor='val_loss', mode='min', verbose=1, save_best_only = True)
y03_Model = load_model('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_p03_parameter.h5')
...
# Selected Feature
corr_pears_p04_para = x_data.corrwith(Train['p04_Parameter'], method='pearson')
corr_pears_p04_para = abs(corr_pears_p04_para)
indices_04 = np.argsort(corr_pears_p04_para)[::-1]

Feature_04 = []
Correlation_feature_04 = []
for f in range(x_data.shape[1]):
    print("%2d) %-*s %f" % (f + 1, 30, 
                            feature_names[indices_04[f]], corr_pears_p04_para[indices_04[f]]))

    Feature_04.append(feature_names[indices_04[f]])
    Correlation_feature_04.append(corr_pears_p04_para[indices_04[f]])
...
for i in range(kfold):
    ...
    es = EarlyStopping(monitor = 'val_loss', patience=30, mode= 'min', min_delta=0.001, verbose=1)
    mc = ModelCheckpoint('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_p04_parameter.h5', 
                            monitor='val_loss', mode='min', verbose=1, save_best_only=True)

    best_model_y04 = keras_model_04()
    best_model_y04.fit(partial_train_data_04, partial_train_targets_04, epochs=30, batch_size=9000)
    best_model_y04.save('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_p04_parameter.h5')
...
mc=ModelCheckpoint('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_p04_parameter.h5', 
                        monitor='val_loss', mode='min', verbose=1, save_best_only = True)
y04_Model = load_model('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_p04_parameter.h5')
...
# Selected Feature
corr_pears_p05_para = x_data.corrwith(Train['p05_Parameter'], method='pearson')
corr_pears_p05_para = abs(corr_pears_p05_para)
indices_05 = np.argsort(corr_pears_p05_para)[::-1]

Feature_05 = []
Correlation_feature_05 = []
for f in range(x_data.shape[1]):
    print("%2d) %-*s %f" % (f + 1, 30, 
                            feature_names[indices_05[f]], corr_pears_p05_para[indices_05[f]]))

    Feature_05.append(feature_names[indices_05[f]])
    Correlation_feature_05.append(corr_pears_p05_para[indices_05[f]])
...
for i in range(kfold):
    ...
    es = EarlyStopping(monitor = 'val_loss', patience=30, mode= 'min', min_delta=0.001, verbose=1)
    mc = ModelCheckpoint('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_p05_parameter.h5', 
                            monitor='val_loss', mode='min', verbose=1, save_best_only=True)

    best_model_y05 = keras_model_05()
    best_model_y05.fit(partial_train_data_05, partial_train_targets_05, epochs=30, batch_size=9000)
    best_model_y05.save('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_p05_parameter.h5')
...
mc=ModelCheckpoint('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_p05_parameter.h5', 
                        monitor='val_loss', mode='min', verbose=1, save_best_only = True)
y05_Model = load_model('C:\\System_Air\\best_model\\physics_parameter\\Pearson20_p05_parameter.h5')
...
with open('Result_Estimation.csv', 'a', newline="") as f:
    writer = csv.writer(f)
    writer.writerow(['Pearson_Case02_P01', loss_y01, acc_y01, Result_R2_y01, Result_RMSE_y01, Result_MAE_y01, Result_MBE_y01])
    writer.writerow(['Pearson_Case02_P02', loss_y02, acc_y02, Result_R2_y02, Result_RMSE_y02, Result_MAE_y02, Result_MBE_y02])
    writer.writerow(['Pearson_Case02_P03', loss_y03, acc_y03, Result_R2_y03, Result_RMSE_y03, Result_MAE_y03, Result_MBE_y03])
    writer.writerow(['Pearson_Case02_P04', loss_y04, acc_y04, Result_R2_y04, Result_RMSE_y04, Result_MAE_y04, Result_MBE_y04])
    writer.writerow(['Pearson_Case02_P05', loss_y05, acc_y05, Result_R2_y05, Result_RMSE_y05, Result_MAE_y05, Result_MBE_y05])
...
with open('ResultPower_Estimation.csv', 'a', newline="") as f:
    writer = csv.writer(f)
    writer.writerow(['Pearson_Power_Case02_P01', ResultPower_R2_y01, ResultPower_RMSE_y01, ResultPower_MAE_y01, ResultPower_MBE_y01])
    writer.writerow(['Pearson_Power_Case02_P02', ResultPower_R2_y02, ResultPower_RMSE_y02, ResultPower_MAE_y02, ResultPower_MBE_y02])
    writer.writerow(['Pearson_Power_Case02_P03', ResultPower_R2_y03, ResultPower_RMSE_y03, ResultPower_MAE_y03, ResultPower_MBE_y03])
    writer.writerow(['Pearson_Power_Case02_P04', ResultPower_R2_y04, ResultPower_RMSE_y04, ResultPower_MAE_y04, ResultPower_MBE_y04])
    writer.writerow(['Pearson_Power_Case02_P05', ResultPower_R2_y05, ResultPower_RMSE_y05, ResultPower_MAE_y05, ResultPower_MBE_y05])
