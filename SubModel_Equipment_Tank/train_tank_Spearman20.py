...
# Selected Feature
corr_spear_tank_para = x_data.corrwith(Train['total_Uas'], method='spearman')   # Spearman Correlation
corr_spear_tank_para = abs(corr_spear_tank_para)
indices_01 = np.argsort(corr_spear_tank_para)[::-1]
...
for f in range(x_data.shape[1]):
   print("%2d) %-*s %f" % (f + 1, 30, 
                            feature_names[indices_01[f]], corr_spear_tank_para[indices_01[f]]))

   Feature_01.append(feature_names[indices_01[f]])
   Correlation_feature_01.append(corr_spear_tank_para[indices_01[f]])


# Save to the Pickle Importance Dataset
with open("corr_spear_tank_para.pickle", "wb") as f:
    pickle.dump(corr_spear_tank_para, f)
...
for i in range(kfold):
    ...
    mc = ModelCheckpoint('C:\\System_Air\\best_model\\physics_parameter\\Spearman20_Tank_parameter.h5', 
    ...
    best_model_y01.save('C:\\System_Air\\best_model\\physics_parameter\\Spearman20_Tank_parameter.h5')
...
mc=ModelCheckpoint('C:\\System_Air\\best_model\\physics_parameter\\Spearman20_Tank_parameter.h5', 
                        monitor='val_loss', mode='min', verbose=1, save_best_only = True)
y01_Model = load_model('C:\\System_Air\\best_model\\physics_parameter\\Spearman20_Tank_parameter.h5')
...
with open('Result_Estimation.csv', 'a', newline="") as f:
    writer = csv.writer(f)
    writer.writerow(['Spearman_Case02_Tank_US', loss, acc, Result_Para_R2, Result_Para_RMSE, Result_Para_MAE, Result_Para_MBE])
...
with open('ResultTempTank_Estimation.csv', 'a', newline="") as f:
    writer = csv.writer(f)
    writer.writerow(['Spearman_Case02_Layer01', Result_Layer01_R2, Result_Layer01_RMSE, Result_Layer01_MAE, Result_Layer01_MBE])
    writer.writerow(['Spearman_Case02_Layer10', Result_Layer10_R2, Result_Layer10_RMSE, Result_Layer10_MAE, Result_Layer10_MBE])
